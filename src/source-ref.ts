...
export function analyzeSourceRef(sourceRef: string): SourceRefAnalysis {
  ...
  pushIf(
    recognizedRefs,
    "pr",
    /(?:^|[\s/])(?:pr|pull request)\s*(?::\s*)?#?\d+\b/i.test(sourceRef),
  );
  ...
  pushIf(
    recognizedRefs,
    "doc_path",
    /(?:^|[\\/])(?:docs|src|tests|config)[\\/][^\s]+|[^\s]+\.(?:md|ts|tsx|js|json|toml)\b/i.test(
      sourceRef,
    ),
  );
  ...
  pushIf(
    recognizedRefs,
    "named_run",
    /\b(?:chat|evaluation|smoke|test)[:-][a-z0-9_.-]+/i.test(sourceRef),
  );
