...
- `start_memory_session` が返す `memoryFreshness` / `memoryUpdateCandidates` / `autoMemoryCuration` を確認し、最新メモリ更新が最近のIssue・PR・commit・session activity・設計判断に追いついていない場合は保存候補として扱います。
...
- 古くなった通常メモリを通常利用から外す場合は `forget_memory` を使い、理由を `invalidation_reason` として残します。根拠が追跡できる場合は `invalidatedByRef` に `issue:#123`、`pr:#123`、`git:<hash>` などを渡します。
